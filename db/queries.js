...
const getLastInsertedId = async (tableName) => {
	return new Promise(async (resolve, reject) => {
		await db.get(`SELECT * FROM ${tableName} ORDER BY id DESC LIMIT 1`, async (err, row) => {
			if (err) {
				reject(err);
			}
			resolve(row.id);
...
const createNewQuiz = async (title, accessCode) => {
	try {
		const query = await db.prepare(`INSERT INTO quizzes (title, accessCode) VALUES (?, ?)`);
...
const createNewQuestion = async (title, description, quizId) => {
	try {
		const query = await db.prepare(`INSERT INTO questions (title, quizID, questionText) VALUES (?, ?, ?)`);
...
const createNewOption = async (min, max, answer, questionId, isCorrect) => {
	return new Promise((resolve, reject) => {
		try {
			const query = db.prepare('INSERT INTO options (questionId, min, max, answer, isCorrect) VALUES (?, ?, ?, ?, ?)');
